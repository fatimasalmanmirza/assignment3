def customer_melons_report(the_file):
    ...
    for line in the_file:
        ...
        customer_firstname =  customer_name.split(" ")[0]
        count = int(words[2])
        customer_paid = float(words[3])
        customer_expected = count * melon_cost
        if customer_expected > customer_paid:
            print(customer_firstname, "paid {}, expected {}".format(customer_paid, customer_expected))
customer_melons_report(the_file = open("customer-orders.txt"))        
    # customer_name = input("customer_name?")
    # count = input("customermelons?")
    # customer_paid = ("customer payment?")
